...
Базовый ROS 2 Jazzy уже должен быть установлен (урок 1). Добавляем сам симулятор и интеграцию:
...
`gz: command not found`. Метапакет не доустановил `gz-tools2`:

```bash
sudo apt install gz-tools2
```
...
Этот флаг необходимо использовать и для всех последующих запусков, если базовый запуск не работает.
...
* `gz sim --version` показывает Harmonic (8.x).
* `gz topic -l` возвращает список топиков пустого мира.
* `ros2 run ros_gz_bridge parameter_bridge` запускается без ошибок и `/clock` тикает в `ros2 topic echo`.

## Дальше

Переходите к статье [Первый мир и gz CLI](./02-first-world.md).
