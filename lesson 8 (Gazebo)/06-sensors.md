...
**`ros2 topic list` не показывает `/imu` или `/scan`.** Сенсор не активирован в Gazebo. Проверьте:
1. Системные плагины в SDF мира (Imu, Sensors).
2. Тег `<sensor>` обёрнут в `<gazebo reference="...">`.
3. Имя линка в `reference` совпадает с реальным линком.

**Лидар работает медленно или зависает Gazebo.** GPU lidar требует видеокарту. На виртуалке или без GPU — замените `gpu_lidar` на `ray` (CPU версия, медленнее).

**IMU даёт постоянный шум даже когда робот стоит.** Это нормально — мы сами включили гауссов шум. Реальный IMU тоже шумит. Для отладки можно временно убрать `<noise>`.

**Точки лидара видны но не в правильном месте.** Frame mismatch. Проверьте `gz_frame_id` сенсора и `frame_id` в LaserScan сообщении — должно быть имя линка где стоит сенсор.
